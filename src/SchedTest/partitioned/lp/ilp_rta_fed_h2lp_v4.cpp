...
uint64_t H2LPMapper_v4::encode_request(uint64_t task_id, uint64_t res_id,
...
uint64_t H2LPMapper_v4::get_type(uint64_t var) {
  return (var >> 36) & (uint64_t)0xf;  // 4 bits
}

uint64_t H2LPMapper_v4::get_task(uint64_t var) {
  return (var >> 26) & (uint64_t)0x3ff;  // 10 bits
}

uint64_t H2LPMapper_v4::get_res_id(uint64_t var) {
  return (var >> 16) & (uint64_t)0x3ff;  // 10 bits
}

uint64_t H2LPMapper_v4::get_req_id(uint64_t var) {
  return var & (uint64_t)0xffff;  // 16 bits
}

H2LPMapper_v4::H2LPMapper_v4(uint start_var) : VarMapperBase(start_var) {}

uint64_t H2LPMapper_v4::lookup(uint64_t task_id, uint64_t res_id, uint64_t req_id, var_type type) {
  uint64_t key = encode_request(task_id, res_id, req_id, type);
  uint64_t var = var_for_key(key);
  return var;
}

string H2LPMapper_v4::key2str(uint64_t key, uint64_t var) const {
  ostringstream buf;

  switch (get_type(key)) {
    case H2LPMapper_v4::REQUEST_NUM:
      buf << "Y[";
      break;
    case H2LPMapper_v4::BLOCKING_SPIN:
      buf << "bs[";
      break;
    case H2LPMapper_v4::BLOCKING_DIRECT:
      buf << "bd[";
      break;
    case H2LPMapper_v4::BLOCKING_INDIRECT:
      buf << "bi[";
      break;
    case H2LPMapper_v4::BLOCKING_PREEMPT:
      buf << "bp[";
      break;
    case H2LPMapper_v4::BLOCKING_TRANS_DIRECT:
      buf << "btd[";
      break;
    case H2LPMapper_v4::BLOCKING_TRANS_INDIRECT:
      buf << "bti[";
      break;
    case H2LPMapper_v4::BLOCKING_TRANS_PREEMPT:
      buf << "btp[";
      break;
    case H2LPMapper_v4::BLOCKING_TRANS_REGULAR:
      buf << "btr[";
      break;
    case H2LPMapper_v4::INTERF:
      buf << "I[";
      break;
    default:
      buf << "?[";
...
/** Class ILP_RTA_FED_H2LP_v4 */

ILP_RTA_FED_H2LP_v4::ILP_RTA_FED_H2LP_v4()
    : PartitionedSched(false, RTA, FIX_PRIORITY, FMLP, "", "") {}

ILP_RTA_FED_H2LP_v4::ILP_RTA_FED_H2LP_v4(DAG_TaskSet tasks,
...
bool ILP_RTA_FED_H2LP_v4::alloc_schedulable() {}

// Algorithm 1 Scheduling Test
// bool ILP_RTA_FED_H2LP_v4::is_schedulable() {
...
bool ILP_RTA_FED_H2LP_v4::is_schedulable() {
  ...
  if (true) { // _ALLOW_LIGHT_TASKS_
    ...
    foreach(tasks.get_tasks(), ti) {
      if (ti->get_other_attr() == 1)  // skip heavy tasks
          continue;
      // calcuate WCRT by fixed-point iteration    
    ...
    return true;  // if no light task is unschedulable
  }

  return false;
}

uint64_t ILP_RTA_FED_H2LP_v4::blocking_spin(const DAG_Task& ti,
                                                         uint32_t res_id) {
  uint64_t sum = 0;
...
uint64_t ILP_RTA_FED_H2LP_v4::blocking_spin_total(const DAG_Task& ti) {
  uint64_t sum = 0;
...
uint64_t ILP_RTA_FED_H2LP_v4::NCS(const DAG_Task& ti, uint32_t res_id) {
  uint32_t sum = 0;
...
uint64_t ILP_RTA_FED_H2LP_v4::workload_bound(const DAG_Task& ti, const DAG_Task& tx) {
...
uint64_t ILP_RTA_FED_H2LP_v4::get_response_time(const DAG_Task& ti) {
  uint64_t C_i = ti.get_wcet();
  uint64_t L_i = ti.get_critical_path_length();
  uint64_t D_i = ti.get_deadline();
...
void ILP_RTA_FED_H2LP_v4::objective(const DAG_Task& ti, H2LPMapper_v4* vars, LinearExpression* obj) {
  int64_t var_id;
  ...
  foreach(tasks.get_tasks(), tx) {
    uint x = tx->get_id();
    var_id = vars->lookup(x, 0, 0, H2LPMapper_v4::INTERF);
    ...
    foreach(tx->get_requests(), request) {
      ...
      for (uint v = 0; v < request_num; v++) {
        var_id = vars->lookup(x, q, v, H2LPMapper_v4::BLOCKING_TRANS_REGULAR);
...
void ILP_RTA_FED_H2LP_v4::declare_variable_bounds(const DAG_Task& ti, LinearProgram* lp,
                                      H2LPMapper_v4* vars) {
  int64_t var_id;
  foreach(tasks.get_tasks(), tx) {
    var_id = vars->lookup(tx->get_id(), 0, 0, H2LPMapper_v4::INTERF);
    lp->declare_variable_bounds(var_id, true, 0, false, -1);
  }

  foreach(ti.get_requests(), request) {
    var_id = vars->lookup(ti.get_id(), request->get_resource_id(), 0, H2LPMapper_v4::REQUEST_NUM);
...
void ILP_RTA_FED_H2LP_v4::lp_h2lp_add_constraints(const DAG_Task& ti, LinearProgram* lp,
                              H2LPMapper_v4* vars) {
  // cout << "C1" << endl;
...
void ILP_RTA_FED_H2LP_v4::spining_blocking(const DAG_Task& ti,
  ...
  foreach(tasks.get_tasks(), tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      if (x == ti.get_id())
        ...
        request_num = tx->get_max_request_num_nu(q, ti.get_deadline());
      for (uint v = 0; v < request_num; v++) {
        var_id = vars->lookup(x, q, v, H2LPMapper_v4::BLOCKING_SPIN);
...
void ILP_RTA_FED_H2LP_v4::direct_blocking(const DAG_Task& ti,
                        H2LPMapper_v4* vars, LinearExpression* exp,
                        double coef) {
  uint64_t var_id;
  foreach(tasks.get_tasks(), tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      if (x == ti.get_id())
        ...
        request_num = tx->get_max_request_num_nu(q, ti.get_deadline());
      for (uint v = 0; v < request_num; v++) {
        var_id = vars->lookup(x, q, v, H2LPMapper_v4::BLOCKING_DIRECT);
        exp->add_term(var_id, coef * length);

        var_id = vars->lookup(x, q, v, H2LPMapper_v4::BLOCKING_TRANS_DIRECT);
        exp->add_term(var_id, coef * length);
      }
    }
  }
...
void ILP_RTA_FED_H2LP_v4::indirect_blocking(const DAG_Task& ti,
                          H2LPMapper_v4* vars, LinearExpression* exp, 
                          double coef) {
  uint64_t var_id;
  foreach(tasks.get_tasks(), tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      if (x == ti.get_id())
        ...
        request_num = tx->get_max_request_num_nu(q, ti.get_deadline());
      for (uint v = 0; v < request_num; v++) {
        var_id = vars->lookup(x, q, v, H2LPMapper_v4::BLOCKING_INDIRECT);
        exp->add_term(var_id, coef * length);

        var_id = vars->lookup(x, q, v, H2LPMapper_v4::BLOCKING_TRANS_INDIRECT);
        exp->add_term(var_id, coef * length);
      }
    }
  }
}
void ILP_RTA_FED_H2LP_v4::preemption_blocking(const DAG_Task& ti,
                          H2LPMapper_v4* vars,
  ...
  foreach(tasks.get_tasks(), tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      if (x == ti.get_id())
        ...
        request_num = tx->get_max_request_num_nu(q, ti.get_deadline());
      for (uint v = 0; v < request_num; v++) {
        var_id = vars->lookup(x, q, v, H2LPMapper_v4::BLOCKING_PREEMPT);
        exp->add_term(var_id, coef * length);

        var_id = vars->lookup(x, q, v, H2LPMapper_v4::BLOCKING_TRANS_PREEMPT);
        exp->add_term(var_id, coef * length);
      }
    }
  }
...
void ILP_RTA_FED_H2LP_v4::constraint_1(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
...
void ILP_RTA_FED_H2LP_v4::constraint_2(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
...
void ILP_RTA_FED_H2LP_v4::constraint_3(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  ...
  foreach_task_except(tasks.get_tasks(), ti, tx) { // for each other cluster for heavy task
    ...
    foreach(tx->get_requests(), request) {
      ...
      if (ti.is_request_exist(q)) {
        var_id = vars->lookup(ti.get_id(), q, 0, H2LPMapper_v4::REQUEST_NUM);
  ...
  for(uint p_id = 0; p_id < p_num_lu; p_id++) { // for each other cluster for light tasks
    ...
    foreach(resources.get_resources(), resource) {
      ...
      foreach(tasks.get_tasks(), tx) {
        ...
        foreach_request_instance_nu(ti, (*tx), q, v) {
          var_id = vars->lookup(x,q,v,H2LPMapper_v4::BLOCKING_SPIN);
          exp->add_term(var_id);
        }
      }
...
void ILP_RTA_FED_H2LP_v4::constraint_4(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  uint i = ti.get_id();
  ...
  foreach(ti.get_requests(), request) {
    ...
    for (uint v = 0; v < request->get_num_requests(); v++) {
      var_id = vars->lookup(i, q, v, H2LPMapper_v4::BLOCKING_DIRECT);
      exp->add_term(var_id);
    }

    var_id = vars->lookup(i, q, 0, H2LPMapper_v4::REQUEST_NUM);
    exp->add_term(var_id);
...
void ILP_RTA_FED_H2LP_v4::constraint_5(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  if (1 == ti.get_other_attr())
    return;
...
void ILP_RTA_FED_H2LP_v4::constraint_6(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  foreach_task_except(tasks.get_tasks(), ti, tx) { // for each other cluster for heavy task
    uint x = tx->get_id();
...
void ILP_RTA_FED_H2LP_v4::constraint_7(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
    ...
    foreach(tx->get_requests(), request) {
      ...
      foreach_lower_priority_local_task(tasks.get_tasks(), ti, ty) {
        ...
        for (uint u = 0; u < request_num_2; u++) {
          var_id = vars->lookup(y,q,u,H2LPMapper_v4::BLOCKING_PREEMPT);
          exp->add_term(var_id, -1.0);
        }
      }
      lp->add_inequality(exp, 0);
...
void ILP_RTA_FED_H2LP_v4::constraint_8(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  LinearExpression *exp = new LinearExpression();
...
void ILP_RTA_FED_H2LP_v4::constraint_9(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  LinearExpression *exp = new LinearExpression();
...
void ILP_RTA_FED_H2LP_v4::constraint_10(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  LinearExpression *exp = new LinearExpression();
  ...
  foreach(ti.get_requests(), request) {
    uint q = request->get_resource_id();
    for (uint v = 0; v < request->get_num_requests(); v++) {
      var_id = vars->lookup(ti.get_id(), q, v, H2LPMapper_v4::BLOCKING_SPIN);
      exp->add_term(var_id);
    }
  }
  if (1 == ti.get_other_attr()) {
...
void ILP_RTA_FED_H2LP_v4::constraint_11(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  LinearExpression *exp = new LinearExpression();
  foreach(tasks.get_tasks(), tx) {
...
void ILP_RTA_FED_H2LP_v4::constraint_12(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  LinearExpression *exp = new LinearExpression();
...
void ILP_RTA_FED_H2LP_v4::constraint_13(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  LinearExpression *exp = new LinearExpression();
...
void ILP_RTA_FED_H2LP_v4::constraint_14(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  LinearExpression *exp = new LinearExpression();
  ...
  foreach(ti.get_requests(), request) {
    uint q = request->get_resource_id();
    for (uint v = 0; v < request->get_num_requests(); v++) {
        var_id = vars->lookup(ti.get_id(), q, v, H2LPMapper_v4::BLOCKING_TRANS_DIRECT);
        exp->add_term(var_id);

        var_id = vars->lookup(ti.get_id(), q, v, H2LPMapper_v4::BLOCKING_TRANS_INDIRECT);
        exp->add_term(var_id);

        var_id = vars->lookup(ti.get_id(), q, v, H2LPMapper_v4::BLOCKING_TRANS_PREEMPT);
        exp->add_term(var_id);

        var_id = vars->lookup(ti.get_id(), q, v, H2LPMapper_v4::BLOCKING_TRANS_REGULAR);
        exp->add_term(var_id);
    }
  }
  if (1 == ti.get_other_attr()) {
...
void ILP_RTA_FED_H2LP_v4::constraint_15(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper_v4* vars) {
  int64_t var_id;
  LinearExpression *exp = new LinearExpression();
  foreach(tasks.get_tasks(), tx) {
...
///////////////////////////// FF //////////////////////////////
/*
ILP_RTA_FED_H2LP_v4_FF::ILP_RTA_FED_H2LP_v4_FF()
    : ILP_RTA_FED_H2LP_v4() {}

ILP_RTA_FED_H2LP_v4_FF::ILP_RTA_FED_H2LP_v4_FF(DAG_TaskSet tasks,
                                         ProcessorSet processors,
                                         ResourceSet resources)
    : ILP_RTA_FED_H2LP_v4(tasks, processors, resources) {
  // this->tasks = tasks;
  // this->processors = processors;
  // this->resources = resources;
...
// Algorithm 1 Scheduling Test
bool ILP_RTA_FED_H2LP_v4_FF::is_schedulable() {
  uint32_t p_num = processors.get_processor_num();
  p_num_lu = p_num;
  uint64_t b_sum = 0;
// cout << "111" << endl;
...
ILP_RTA_FED_H2LP_v4_UTIL::ILP_RTA_FED_H2LP_v4_UTIL()
    : ILP_RTA_FED_H2LP_v4() {}

ILP_RTA_FED_H2LP_v4_UTIL::ILP_RTA_FED_H2LP_v4_UTIL(DAG_TaskSet tasks,
                                         ProcessorSet processors,
                                         ResourceSet resources)
    : ILP_RTA_FED_H2LP_v4(tasks, processors, resources) {
}
...
// Algorithm 1 Scheduling Test
// bool ILP_RTA_FED_H2LP_v4_HEAVY::is_schedulable() {
//   uint32_t p_num = processors.get_processor_num();
//   p_num_lu = p_num;
//   uint64_t b_sum = 0;
//   foreach(tasks.get_tasks(), ti) {
...
bool ILP_RTA_FED_H2LP_v4_UTIL::is_schedulable() {
  uint32_t p_num = processors.get_processor_num();
  p_num_lu = p_num;
  uint64_t b_sum = 0;
  foreach(tasks.get_tasks(), ti) {
  ...
  uint32_t sum_core_heavy = 0;

  foreach(tasks.get_tasks(), ti) {
    if (ti->get_utilization() <= 1)  // skip low-utilization tasks
      continue;
    uint64_t D_i = ti->get_deadline();
    uint64_t temp = get_response_time((*ti));
    // get_response_time_HUT_2((*ti));
    ...
    sum_core_heavy += ti->get_dcores();
  }

  if (sum_core_heavy > p_num) {
    return false;
  }
  ...
  if (true) { // _ALLOW_LIGHT_TASKS_
    foreach(tasks.get_tasks(), ti) {
        if (ti->get_utilization() > 1)  // skip high-utilization tasks
          continue;
        ti->set_other_attr(0);
    }

    p_num_lu = p_num - sum_core_heavy;
    if (0 == p_num_lu) {
      return false;
    }
    ...
    return true;  // if no low-utilization task is unschedulable
  }
...
ILP_RTA_FED_H2LP_v4_HEAVY::ILP_RTA_FED_H2LP_v4_HEAVY()
    : ILP_RTA_FED_H2LP_v4() {}

ILP_RTA_FED_H2LP_v4_HEAVY::ILP_RTA_FED_H2LP_v4_HEAVY(DAG_TaskSet tasks,
                                         ProcessorSet processors,
                                         ResourceSet resources)
    : ILP_RTA_FED_H2LP_v4(tasks, processors, resources) {
}
