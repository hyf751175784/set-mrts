...
LP_RTA_PFP_ROP_DPCP::LP_RTA_PFP_ROP_DPCP()
    : PartitionedSched(false, RTA, FIX_PRIORITY, DPCP, "",
                       "Resource-Oriented") {}

LP_RTA_PFP_ROP_DPCP::LP_RTA_PFP_ROP_DPCP(TaskSet tasks, 
...
// ROP
ulong LP_RTA_PFP_ROP_DPCP::blocking_bound(const Task& ti, uint r_id) {
  ulong bound = 0;
...
ulong LP_RTA_PFP_ROP_DPCP::request_bound(const Task& ti, uint r_id) {
...
ulong LP_RTA_PFP_ROP_DPCP::formula_30(const Task& ti, uint p_id,
                                      ulong interval) {
  ulong miu = 0;
...
ulong LP_RTA_PFP_ROP_DPCP::angent_exec_bound(const Task& ti, uint p_id,
                                             ulong interval) {
  ulong deadline = ti.get_deadline();
  ulong lambda = 0;
  foreach(ti.get_requests(), request) {
...
ulong LP_RTA_PFP_ROP_DPCP::NCS_workload(const Task& ti, ulong interval) {
...
ulong LP_RTA_PFP_ROP_DPCP::CS_workload(const Task& ti, uint resource_id,
                                       ulong interval) {
  ulong p = ti.get_period();
  ulong r = ti.get_response_time();
...
ulong LP_RTA_PFP_ROP_DPCP::response_time_AP(const Task& ti) {
...
ulong LP_RTA_PFP_ROP_DPCP::response_time_SP(const Task& ti) {
  uint p_id = ti.get_partition();
  ulong test_bound = ti.get_deadline();
  ulong test_start = ti.get_wcet_non_critical_sections();
  ulong response_time = test_start;
...
bool LP_RTA_PFP_ROP_DPCP::worst_fit_for_resources(uint active_processor_num) {
  /*
...
bool LP_RTA_PFP_ROP_DPCP::is_first_fit_for_tasks_schedulable(
...
// DPCP
ulong LP_RTA_PFP_ROP_DPCP::local_blocking(Task* ti) {
...
ulong LP_RTA_PFP_ROP_DPCP::remote_blocking(Task* ti) {
...
ulong LP_RTA_PFP_ROP_DPCP::total_blocking(Task* ti) {
...
ulong LP_RTA_PFP_ROP_DPCP::interference(const Task& ti, ulong interval) {
  return ti.get_wcet() *
         ceiling((interval + ti.get_response_time()), ti.get_period());
}

ulong LP_RTA_PFP_ROP_DPCP::response_time(Task* ti) {
...
// bool LP_RTA_PFP_ROP_DPCP::alloc_schedulable() {
//   bool update = false;
...
ulong LP_RTA_PFP_ROP_DPCP::get_max_wait_time(const Task& ti,
                                             const Request& rq) {
...
void LP_RTA_PFP_ROP_DPCP::lp_dpcp_objective(const Task& ti, LinearProgram* lp,
                                            DPCPMapper* vars,
...
void LP_RTA_PFP_ROP_DPCP::lp_dpcp_add_constraints(const Task& ti,
                                                  LinearProgram* lp,
                                                  DPCPMapper* vars) {
...
void LP_RTA_PFP_ROP_DPCP::lp_dpcp_constraint_1(const Task& ti,
                                               LinearProgram* lp,
                                               DPCPMapper* vars) {
...
void LP_RTA_PFP_ROP_DPCP::lp_dpcp_constraint_2(const Task& ti,
                                               LinearProgram* lp,
                                               DPCPMapper* vars) {
  LinearExpression* exp = new LinearExpression();
  ...
  foreach_task_except(tasks.get_tasks(), ti, tx) {
    ...
    foreach_remote_request(ti, tx->get_requests(), request_iter) {
      // cout << "request locality" << request_iter->get_locality() << endl;
...
void LP_RTA_PFP_ROP_DPCP::lp_dpcp_constraint_3(const Task& ti,
                                               LinearProgram* lp,
                                               DPCPMapper* vars) {
...
void LP_RTA_PFP_ROP_DPCP::lp_dpcp_constraint_4(const Task& ti,
                                               LinearProgram* lp,
                                               DPCPMapper* vars) {
  LinearExpression* exp = new LinearExpression();
  uint priority = ti.get_priority();
...
void LP_RTA_PFP_ROP_DPCP::lp_dpcp_constraint_5(const Task& ti,
                                               LinearProgram* lp,
                                               DPCPMapper* vars) {
  uint priority = ti.get_priority();
...
void LP_RTA_PFP_ROP_DPCP::lp_dpcp_constraint_6(const Task& ti,
                                               LinearProgram* lp,
                                               DPCPMapper* vars) {
  ulong max_wait_time_l = 0;
  ulong max_wait_time_h = 0;
...
bool LP_RTA_PFP_ROP_DPCP::alloc_schedulable(Task* ti) {
  uint p_id = ti->get_partition();
...
bool LP_RTA_PFP_ROP_DPCP::is_schedulable() {
  ...
  for (uint i = 1; i <= p_num; i++) {
    ...
    //   foreach(ti->get_requests(), request) {
    //     cout << "request:" << request->get_resource_id()
    //          << " locality:" << request->get_locality() <<endl;
    //   }
    // }
