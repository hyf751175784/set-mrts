...
uint64_t H2LPMapper::encode_request(uint64_t task_id, uint64_t res_id,
...
uint64_t H2LPMapper::get_task(uint64_t var) {
...
uint H2LPMapper::lookup(uint task_id, uint res_id, uint req_id, var_type type) {
...
string H2LPMapper::key2str(uint64_t key, uint var) const {
  ...
  switch (get_type(key)) {
    ...
    case H2LPMapper::BLOCKING_OTHER:
      buf << "Xo[";
      break;
    case H2LPMapper::INTERF_REGULAR:
      buf << "I[";
      break;
...
uint64_t ILP_RTA_FED_H2LP::get_response_time_HUT(const DAG_Task& ti) {
  ...
  cout << "declare_variable_bounds" << endl; 
  vars.seal();
  ...
  if (rb_solution->is_solved()) {
    ...
    foreach(ti.get_requests(), r_i_q) {
      ...
      sum += max;
      cout << "max_y[" << r_id << "]:" << max_y << endl;
    }

    response_time += sum + (C_i + B_spin_total - L_i) / n_i;

    cout << "Intra_interf: " << (C_i + B_spin_total - L_i) << endl;
    cout << "sum:" << sum << endl;
    cout<<"original response time:"<<response_time<<endl;
    cout << "L_i:" << ti.get_critical_path_length() << endl;
    ...
    foreach(ti.get_requests(), request) {
      ...
      exp->add_term(vars.lookup(ti.get_id(), request->get_resource_id(), 0, H2LPMapper::REQUEST_NUM));
    ...
    exp = new LinearExpression();
    exp->add_term(vars.lookup(ti.get_id(), 0, 0, H2LPMapper::INTERF_REGULAR));
    result = rb_solution->evaluate(*exp);
    ...
    foreach_task_except(tasks.get_tasks(), ti, tx) {
      uint x = tx->get_id();
      exp->add_term(vars.lookup(x, 0, 0, H2LPMapper::INTERF_REGULAR));
    }
    result = rb_solution->evaluate(*exp);
    cout << "Inter_interf: " << result << endl;
    delete(exp);

    exp = new LinearExpression();
    foreach_task_except(tasks.get_tasks(), ti, tx) {
      uint x = tx->get_id();
      exp->add_term(vars.lookup(x, 0, 0, H2LPMapper::INTERF_OTHER));
    }
    result = rb_solution->evaluate(*exp);
    cout << "Inter_interf_other: " << result << endl;
    delete(exp);
    ...
    foreach(ti.get_requests(), request) {
      ...
      for (uint v = 0; v < request->get_num_requests(); v++) {
        exp->add_term(vars.lookup(ti.get_id(), q, v, H2LPMapper::BLOCKING_DIRECT), length);
      }
    }
    result = rb_solution->evaluate(*exp);
    cout << "Direct_blocing_susp: " << result << endl;
    delete(exp);
...
void ILP_RTA_FED_H2LP::lp_h2lp_directed_blocking(const DAG_Task& ti,
  ...
  foreach(tasks.get_tasks(), tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      uint request_num_bound;
      if (ti.get_id() == tx->get_id())
        request_num_bound = request->get_num_requests();
      else
        request_num_bound = tx->get_max_request_num(q, ti.get_response_time());
      if (request_num_bound >= 1024) {
        cout << "response time of ti:" << ti.get_response_time()
             << " period of tx:" << tx->get_period()
             << " max instance of tx in ti:" << tx->get_max_job_num(ti.get_response_time())
             << " request num:" << request->get_num_requests() << endl;
      }
      for(uint v = 0; v < request_num_bound; v++) {
        var_id = vars->lookup(x, q, v, H2LPMapper::BLOCKING_DIRECT);
...
void ILP_RTA_FED_H2LP::lp_h2lp_indirected_blocking(const DAG_Task& ti,
  ...
  foreach(tasks.get_tasks(), tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      for(uint v = 0; v < request_num_bound; v++) {
        var_id = vars->lookup(x, q, v, H2LPMapper::BLOCKING_INDIRECT);
        exp->add_term(var_id, coef * length);
      }
    }
  }
...
void ILP_RTA_FED_H2LP::lp_h2lp_preemption_blocking(const DAG_Task& ti,
  ...
  foreach(tasks.get_tasks(), tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      for(uint v = 0; v < request_num_bound; v++) {
        var_id = vars->lookup(x, q, v, H2LPMapper::BLOCKING_PREEMPT);
        exp->add_term(var_id, coef * length);
      }
    }
  }
...
void ILP_RTA_FED_H2LP::lp_h2lp_declare_variable_bounds(const DAG_Task& ti, LinearProgram* lp,
  ...
  foreach(tasks.get_tasks(), tx) {
    uint x = tx->get_id();
    var_id = vars->lookup(x, 0, 0, H2LPMapper::INTERF_REGULAR);
    lp->declare_variable_bounds(var_id, true, 0, false, -1);

    var_id = vars->lookup(x, 0, 0, H2LPMapper::INTERF_OTHER);
    lp->declare_variable_bounds(var_id, true, 0, false, -1);
  }

  foreach(ti.get_requests(), request) {
    // LinearExpression* exp = new LinearExpression();
    var_id = vars->lookup(ti.get_id(), request->get_resource_id(), 0, H2LPMapper::REQUEST_NUM);
    ...
    cout << "Y_" << request->get_resource_id() << ": " << "[1," << request->get_num_requests() << "]" << endl;
  }
...
void ILP_RTA_FED_H2LP::lp_h2lp_add_constraints(const DAG_Task& ti, LinearProgram* lp,
  ...
  cout << "C_6" << endl; 
  lp_h2lp_constraint_7(ti, lp, vars);
  cout << "C_7" << endl; 
  lp_h2lp_constraint_8(ti, lp, vars);
  cout << "C_8" << endl; 
  lp_h2lp_constraint_9(ti, lp, vars);
  cout << "C_9" << endl; 
  lp_h2lp_constraint_10(ti, lp, vars);
  cout << "C_10" << endl; 
}
...
void ILP_RTA_FED_H2LP::lp_h2lp_constraint_1(const DAG_Task& ti, LinearProgram* lp,
  ...
  uint var_id;
  uint64_t upper_bound = ti.get_wcet() - ti.get_critical_path_length() ;
  LinearExpression* exp = new LinearExpression();
  ...
  foreach_task_except(tasks.get_tasks(), ti, tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      if (!ti.is_request_exist(q))
        continue;
      uint64_t length = request->get_max_length();
      uint request_num_bound = tx->get_max_request_num(q, ti.get_response_time());
      if (request_num_bound >= 1024) {
        cout << "response time of ti:" << ti.get_response_time()
             << " period of tx:" << tx->get_period()
             << " max instance of tx in ti:" << tx->get_max_job_num(ti.get_response_time())
...
void ILP_RTA_FED_H2LP::lp_h2lp_constraint_4(const DAG_Task& ti, LinearProgram* lp,
  ...
  foreach(ti.get_requests(), request) {
    ...
    LinearExpression* exp = new LinearExpression();
    for(uint v = 0; v < request->get_num_requests(); v++) {
      var_id = vars->lookup(i, q, v, H2LPMapper::BLOCKING_DIRECT);
      exp->add_term(var_id);

      var_id = vars->lookup(i, q, v, H2LPMapper::BLOCKING_INDIRECT);
      exp->add_term(var_id);

      var_id = vars->lookup(i, q, v, H2LPMapper::BLOCKING_PREEMPT);
      exp->add_term(var_id);
    }
    var_id = vars->lookup(i, q, 0, H2LPMapper::REQUEST_NUM);
    exp->add_term(var_id);
    lp->add_inequality(exp, request->get_num_requests());
  }

  pd = ti.get_parallel_degree();
  cout << "parallel degree:" << pd << endl;
  foreach(ti.get_requests(), request) {
    uint q = request->get_resource_id();
    LinearExpression* exp = new LinearExpression();
    for(uint v = 0; v < request->get_num_requests(); v++) {
...
void ILP_RTA_FED_H2LP::lp_h2lp_constraint_5(const DAG_Task& ti, LinearProgram* lp,
  ...
  foreach_task_except(tasks.get_tasks(), ti, tx) {
    uint x = tx->get_id();
    foreach(tx->get_requests(), request) {
      if (!ti.is_request_exist(request->get_resource_id()))
        continue;
      uint q = request->get_resource_id();
      LinearExpression* exp = new LinearExpression();
	    uint request_num_bound = tx->get_max_request_num(q, ti.get_response_time());
  ...
  foreach(tasks.get_tasks(), tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      var_id = vars->lookup(ti.get_id(), q, 0, H2LPMapper::REQUEST_NUM);
...
void ILP_RTA_FED_H2LP::lp_h2lp_constraint_7(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper* vars) {
  uint var_id;
...
void ILP_RTA_FED_H2LP::lp_h2lp_constraint_8(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper* vars) {
  uint var_id;
  uint64_t upper_bound = 0;
  ...
  foreach_task_except(tasks.get_tasks(), ti, tx) {
    ...
    foreach(tx->get_requests(), request) {
      ...
      uint q = request->get_resource_id();
      var_id = vars->lookup(ti.get_id(), q, 0, H2LPMapper::REQUEST_NUM);
      exp->add_term(var_id, request->get_max_length());

      upper_bound += request->get_max_length() * ti.get_request_by_id(q).get_num_requests();
    } 
  }
  lp->add_inequality(exp, upper_bound);
}

void ILP_RTA_FED_H2LP::lp_h2lp_constraint_9(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper* vars) {
  uint var_id;
  uint i = ti.get_id();

  LinearExpression* exp = new LinearExpression();
  var_id = vars->lookup(i, 0, 0, H2LPMapper::INTERF_OTHER);
  exp->add_term(var_id);

  lp->add_equality(exp, 0);
}

void ILP_RTA_FED_H2LP::lp_h2lp_constraint_10(const DAG_Task& ti, LinearProgram* lp,
                          H2LPMapper* vars) {
  uint var_id;
